...
public class PlayerDAO {
	...
	//선수 이미지 업로드
		public PlayerDTO updatePlayerImgUpload(HttpServletRequest request) throws IOException {
			...
			//multipart 타입으로 파라미터를 전송하면 값을 getParameter로 불러올 수 없기 때문에 브라우저로부터 MultipartRequest에
			//저장된 값들을 꺼내서 일일히 PlayerDTO의 필드 값에 할당해주어야 한다.
			//updatePl.jsp로부터 받아온 파라미터 값들
		    String plName=multi.getParameter("plName");	    
		    String plPosition=multi.getParameter("plPosition");	    
		    String plBirth=multi.getParameter("plBirth");	   
		    String plBackNo=multi.getParameter("plBackNo");    
		    String plPhysical=multi.getParameter("plPhysical");	    
		    String plPnH=multi.getParameter("plPnH");	   
		    String plSubject=multi.getParameter("plSubject");	    
		    String plContents=multi.getParameter("plContents");	       
		    String plImgName=multi.getParameter("plImgName");	
		    String plId=multi.getParameter("plId");	
		    String plMemName=multi.getParameter("plMemName");
		    
		    System.out.println("plfileName :: "+plImgName);
			...
			if(files.hasMoreElements()) { //다음 파일 정보가 있으면,
				...
				//새로 업로드 한 파일명을 plImgName 필드에 저장.
				plImgName = fileName;
				
			}
			if(plImgName==null) {
				plImgName=multi.getParameter("plImgName");
			}
			//브라우저로부터 받은 값으로 DTO 생성.
			player = new PlayerDTO(plName,plPosition,plBirth,plBackNo,plPhysical,
					plPnH,plSubject,plContents,plImgName,plId,plMemName);
			System.out.println("fileName :: "+player);
			return player;
